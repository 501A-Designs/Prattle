...
export default function GhenInterpreter(props) {
  ...
  if (setGoogleMap) type = 'google-map';
  if (setBold) type = 'bold-text';      
  if (setItalic) type = 'italic-text';
  if (setColor) type = 'colored-text';      
  if (setDropdown) type = 'dropdown-text';
  if (setReprate) type = 're-prate'
  ...
  return (
    <div>
      ...
      {type === 're-prate' && 
        <div>
          <p>
            <pre>{data.substr(data.indexOf(" ") + 1)}</pre>
          </p>
          <MessageById messageId={shortcutProp && shortcutProp.split('>')[1]}/>
        </div>
      }
      {type === 'bold-text' && <b><pre>{keyData}</pre></b>}
      {type === 'italic-text' && <p><i><pre>{keyData}</pre></i></p>}
      {type === 'colored-text' && 
      ...
      {type === 'google-map' && 
        <>
          <Tag color="blue" icon={<FiMapPin/>}>{shortcutProp}</Tag>
