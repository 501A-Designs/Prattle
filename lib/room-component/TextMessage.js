...
import { FiChevronUp, FiCopy, FiCornerUpLeft, FiFileText } from 'react-icons/fi';
...
export default function TextMessage(props) {
  ...
  let roomCreator = props.roomInfo && props.roomInfo.room_creator;
  let roomEditable = props.roomInfo && props.roomInfo.room_editable;
  ...
  useEffect(async () => {
    if (!props.hideThread) {
      ...
      threads && setMessagesThreadArray(threads);
    }
  },[room])
      ...
      <div className={styles.messageContainer}>
        ...
        />
          <GridItems gap={'0'}>
            ...
            {messagesThreadArray.length > 0 && !messagesThreadDisplay &&
            ...
            {messagesThreadDisplay &&      
              <ThreadContainer
                messagesThreadArray={messagesThreadArray}
                linkedId={props.data.id}
            ...
            {user &&
              <>
                {roomEditable || user.id === roomCreator && <PrateMessageButtons/>}
              </>
            }
            </AlignItems>
        </div>
